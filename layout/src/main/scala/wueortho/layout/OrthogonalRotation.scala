...
object OrthogonalRotation:
  ...
  def layout(graph: WeightedGraph, init: VertexLayout): VertexLayout =
    ...
    val alphaList = graph.edges
      ...
      .map((e1, e2) => math.atan((e1.x1-e2.x1)/(e1.x2-e2.x2)))
    ...
    val thetaList = LazyList.from(1).map(math.Pi * _ - theta).takeWhile(_ < 2*math.Pi)
    ...
    val fx = (t: Double, a: Seq[Double]) => a.map(a => Math.sin(2* (t+a/4)))
      .map(x => x*x)
      .sum

    //computes the minimum and returns the corresponding angle
    val angle = thetaList(thetaList.map(t => fx(t, alphaList)).zipWithIndex.minBy(_._1)._2)
 
    println(s"Angle for rotation: ${angle * (180/Math.PI)}")
    ...
    def rotate(origin: Vec2D, point: Vec2D, angle: Double): Vec2D =
      ...
      val qy = origin.x2 + Math.sin(angle) * deltaP.x2 + Math.cos(angle) * deltaP.x2
      Vec2D(qx, qy)
    end rotate
